...
public class TelaParticipante {
	...
	private ButtonGroup grupobotoes2;
	...
	private void initialize() {
		...
		panel_admin.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, new Color(255, 255, 255), new Color(160, 160, 160)), "Admin", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
		panel_admin.setBounds(296, 202, 107, 61);
		frame.getContentPane().add(panel_admin);
		panel_admin.setLayout(new GridLayout(2, 0));
		
		JRadioButton radioSim = new JRadioButton("Sim");
		panel_admin.add(radioSim);
		
		JRadioButton radioNao = new JRadioButton("Não");
		radioNao.setSelected(true);
		panel_admin.add(radioNao);
		
		grupobotoes2 = new ButtonGroup(); // permite selecao unica dos botoes
		grupobotoes2.add(radioSim);
		grupobotoes2.add(radioNao);
				...
				try {
					...
					if (radioButton_1.isSelected()) {
						if(radioNao.isSelected())
							...
							Fachada.criarAdministrador(nome, senha);
					}
		...
		JButton gerenciarGrupos = new JButton("Gerenciar Grupos");
		gerenciarGrupos.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
